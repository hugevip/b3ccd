...
const char * const BITCOIN_PID_FILENAME = "b3ccd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "b3ccd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\B3CC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\B3CC
    // Mac: ~/Library/Application Support/B3CC
    // Unix: ~/.b3cc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "B3CC";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/B3CC";
#else
    // Unix
    return pathRet / ".b3cc";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("B3CC") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The B3CC developers";
    }
