...
void static B3CCMiner(const CChainParams& chainparams)
{
    LogPrintf("B3CC Miner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("b3cc-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("B3CC Miner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("B3CC Miner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("B3CC Miner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("B3CC Miner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("B3CC Miner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateB3CCs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&B3CCMiner, boost::cref(chainparams)));
    }
