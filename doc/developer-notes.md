...
$ valgrind -v --leak-check=full src/b3ccd -printtoconsole
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8225.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8224 for connections and services them.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `b3cc-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `b3cc-cli` and the GUI debug console use this table to determine how to
