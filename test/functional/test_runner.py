...
def main():
    ...
    enable_b3ccd = config["components"].getboolean("ENABLE_BITCOIND")
...
def run_tests(test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False):
    ...
    # Warn if b3ccd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "b3ccd"]) is not None:
            print("%sWARNING!%s There is already a b3ccd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `b3cc-cli help` (`rpc_interface.txt`).
