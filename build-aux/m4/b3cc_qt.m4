...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$b3cc_qt_want_version" = xauto && test "x$b3cc_qt_force" != xyes; then
    if test "x$bitcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; b3cc-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$bitcoin_enable_qt" != xno && test "x$b3cc_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build b3cc-qt GUI (default=auto)])],
    [
     b3cc_qt_want_version=$withval
     if test "x$b3cc_qt_want_version" = xyes; then
       b3cc_qt_force=yes
       b3cc_qt_want_version=auto
     fi
    ],
    [b3cc_qt_want_version=auto])
...
dnl Inputs: b3cc_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If b3cc_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: b3cc_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$b3cc_qt_want_version" = xauto; then
